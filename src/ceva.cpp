...
template <typename Scalar = double, int RowSize = Dynamic,
          int ColSize = Dynamic>
Matrix<Scalar, RowSize, ColSize> load_dlm(const std::string &path, string dlm,
                                          int r_start = 0, int col_start = 0) {
  std::ifstream indata;
  indata.open(path);
  std::string line;
  ...
  return Map<const Matrix<Scalar, RowSize, ColSize, RowMajor>>(
...
public:
  ...
  Ceva(int N, double dt, double start_time, double final_time) {
    ...
    traj->extendKnotsTo(final_time, Sophus::SE3<double>(Quaternd::Identity(),
                                                        Vector3d(0, 0, 0)));

    printf("Created spline of order %d, knot length %f, from time %f to %f; "
           "consisting of %d knots\n",
  ...
  Ceva(int N, double dt, double start_time, string spline_log) {
    ...
    // Make sure the number of knots inferred from final time matches with the
    // log
...
#pragma omp parallel for num_threads(MAX_THREADS)
    for (int i = 0; i < traj->numKnots(); i++) {
      ...
      Quaternd q(knots_pose(i, 8), knots_pose(i, 5), knots_pose(i, 6),
                 knots_pose(i, 7));

      // printf("Setting knot %5d with pose: %.3f. %.3f. %.3f. %.3f. %.3f. %.3f.
      // %.3f\n",
      //         i,
      //         p.x(), p.y(), p.z(),
      //         q.x(), q.y(), q.z(), q.w());
...
#pragma omp parallel for num_threads(MAX_THREADS)
    for (int i = 0; i < traj->numKnots(); i++) {
      ...
      Quaternd q(knots_pose(i, 6), knots_pose(i, 3), knots_pose(i, 4),
                 knots_pose(i, 5));
...
#pragma omp parallel for num_threads(MAX_THREADS)
    for (int i = 0; i < sample_times.size(); i++) {
      ...
      Vector3d omegainW =
          traj->pose(t).so3().unit_quaternion() * traj->rotVelBody(t);
...
#pragma omp parallel for num_threads(MAX_THREADS)
  ...
  int order() { return traj->order(); }

  double getDt() { return traj->getDt(); }

  double minTime() const { return traj->minTime(); }

  double maxTime() const { return traj->maxTime(); }

  int numKnots() { return traj->numKnots(); }
...
#pragma omp parallel for num_threads(MAX_THREADS)
  ...
  string fitspline(vector<double> ts, MatrixXd pos, MatrixXd rot,
                   vector<double> wp, vector<double> wr, double loss_thres) {
    // Create spline
    ...
    ceres::LocalParameterization *local_parameterization =
        new basalt::LieAnalyticLocalParameterization<Sophus::SO3d>();

    // Add the parameter blocks for rotation
    for (int knot_idx = 0; knot_idx < KNOTS; knot_idx++)
      problem.AddParameterBlock(traj->getKnotSO3(knot_idx).data(), 4,
    ...
    for (int k = 0; k < ts.size(); k++) {
      ...
      ceres::CostFunction *cost_function =
          new PoseAnalyticFactor(SE3d(q, p), wp[k], wr[k], N, traj->getDt(), u);
      ...
      auto res_block = problem.AddResidualBlock(cost_function, loss_function,
                                                factor_param_blocks);
    ...
    string report =
        myprintf("Spline Fitting. Cost: %f -> %f. Iterations: %d.\n",
...
PYBIND11_MODULE(ceva, m) {
  py::class_<Ceva>(m, "Ceva")
      ...
      .def("blendingMatrix",
           static_cast<MatrixXd (Ceva::*)(bool)>(&Ceva::blendingMatrix),
           "Get the blending matrix of the spline.")
      .def("minTime", &Ceva::minTime, "Get the spline minimum time.")
      .def("maxTime", &Ceva::maxTime, "Get the spline maximum time.")
      .def("numKnots", &Ceva::numKnots,
      ...
      .def("getPose",
           static_cast<MatrixXd (Ceva::*)(vector<double>)>(&Ceva::getPose),
           "Get the poses and multiple times.")
      .def("getPose", static_cast<MatrixXd (Ceva::*)(double)>(&Ceva::getPose),
           "Get the pose at one time.")

      .def("getKnotPose",
           static_cast<MatrixXd (Ceva::*)(vector<int>)>(&Ceva::getKnotPose),
           "Get the knot poses by a list of index.")
      .def("getKnotPose",
           static_cast<MatrixXd (Ceva::*)(int)>(&Ceva::getKnotPose),
           "Get the knot pose at one time.")
      .def("getAllKnotPose",
           static_cast<MatrixXd (Ceva::*)()>(&Ceva::getAllKnotPose),
           "Get all the knot poses.")

      .def("getAngularVelWorld",
           static_cast<MatrixXd (Ceva::*)(vector<double>)>(
               &Ceva::getAngularVelWorld),
           "Get the angular velocity in inertial frame at multiple times.")
      .def("getAngularVelWorld",
           static_cast<MatrixXd (Ceva::*)(double)>(&Ceva::getAngularVelWorld),
           "Get the angular velocity in inertial frame at a single time.")

      .def(
          "getTransVelWorld",
          static_cast<MatrixXd (Ceva::*)(vector<double>)>(
              &Ceva::getTransVelWorld),
          "Get the translational velocity in inertial frame at multiple times.")
      .def("getTransVelWorld",
           static_cast<MatrixXd (Ceva::*)(double)>(&Ceva::getTransVelWorld),
           "Get the translational velocity in inertial frame at a single time.")

      .def("getTransAccelWorld",
           static_cast<MatrixXd (Ceva::*)(vector<double>)>(
               &Ceva::getTransAccelWorld),
           "Get the translational acceleration in inertial frame at multiple "
           "times.")
      .def("getTransAccelWorld",
           static_cast<MatrixXd (Ceva::*)(double)>(&Ceva::getTransAccelWorld),
           "Get the translational acceleration in inertial frame at a single "
           "time.")
      ...
      .def("genRandomTrajectory", &Ceva::genRandomTrajectory,
           "Random initialization of the spline.")
      .def(
          "fitspline",
          static_cast<string (Ceva::*)(vector<double>, MatrixXd, MatrixXd,
                                       vector<double>, vector<double>, double)>(
              &Ceva::fitspline),
          "Get a spline that best fits sampling data.")
      .def("deskewCloud",
